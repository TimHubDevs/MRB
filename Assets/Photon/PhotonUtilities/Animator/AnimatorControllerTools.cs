...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class TransitionInfo
    {
        ...
        public TransitionInfo(int index, int hash, int state, int destination, float duration, float offset,
    ...
    public static class AnimatorControllerTools
    {
        ...
        public static void GetTriggerNames(this AnimatorController ctr, List<string> namelist)
        {
            ...
            foreach (var p in ctr.parameters)
                if (p.type == AnimatorControllerParameterType.Trigger)
                {
                    ...
                    {
                        Debug.LogWarning("Idential Trigger Name Found.  Check animator on '" + ctr.name +
                                         "' for repeated trigger names.");
                    }
                    else
                        namelist.Add(p.name);
                }
        ...
        public static void ExtractSubNames(AnimatorController ctr, string path, ChildAnimatorStateMachine[] substates,
        ...
        public static void ExtractNames(AnimatorController ctr, string path, ChildAnimatorState[] states,
            List<string> namelist)
        {
            foreach (var st in states)
            {
                ...
                {
                    Debug.LogWarning("Idential State Name <i>'" + st.state.name + "'</i> Found.  Check animator on '" +
                                     ctr.name + "' for repeated State names as they cannot be used nor networked.");
                }
                else
        ...
        public static void ExtractSubtHashes(AnimatorController ctr, string path, ChildAnimatorStateMachine[] substates,
            List<int> hashlist)
        {
            foreach (var s in substates)
            {
        ...
        public static void ExtractHashes(AnimatorController ctr, string path, ChildAnimatorState[] states,
            List<int> hashlist)
        {
            foreach (var st in states)
            {
